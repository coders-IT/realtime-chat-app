...
const UserState = (props) => {
    ...
    useEffect(() => {
        if (newMsg.sender === chatWith.username) {
            ...
            if (newMsg["read"] == false && newMsg.user1 != userDetail.username) {
                ...
                if (mp.get(newMsg.user1))
                    mp.set(newMsg.user1, mp.get(newMsg.user1) + 1);
    ...
    useEffect(() => {
        ...
        const func = async () => {
            if (
                UserStateChg.username &&
                UserStateChg.username === userDetail.username
            ) {
    ...
    const handleUserStateChg = (user) => {
        const firebaseApp = initializeApp({
            ...
            databaseURL:
                "https://bhannasa-realtime-chat-app-default-rtdb.firebaseio.com",
